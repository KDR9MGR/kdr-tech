...
export default function EditVideoTestimonialPage() {
  ...
  const [testimonial, setTestimonial] = useState<VideoTestimonial | null>(null)
  ...
  const fetchTestimonial = async () => {
    try {
      const response = await fetch(`/api/testimonials/video/${params.id}`)
      if (response.ok) {
  ...
  return (
    <div className="flex min-h-screen bg-[#030014]">
      <AdminSidebar />
      <main className="flex-1 p-8">
        <VideoTestimonialForm initialData={testimonial} isEdit />
      </main>
    </div>
  )
}
